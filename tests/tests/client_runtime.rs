...
use russh_extra::{
    ...
    HostKeyErrorKind, Identity, KnownHosts, Operation, RemoteCommand, TcpEndpoint, Timeouts,
};
...
#[tokio::test]
async fn sftp_open_reports_unsupported_until_runtime_lands() {
    init_tracing();
    let server = LoopbackServer::start(LoopbackServerConfig::new().password("demo", "demo"))
        .await
        .unwrap();
    ...
    let error = session.sftp().await.unwrap_err();

    let Error::Unsupported(message) = error else {
        panic!("expected unsupported error");
    };
    assert!(message.contains("SFTP client is not implemented yet"));
}
